...
of all numbers in the range 1 to n and return an array of size n such that result[i] represents the frequency of the number 
i (1-based indexing).
...
function frequencyCount(array) {
  let hash = new Array(array.length).fill(0);
  for (let i = 0; i < array.length; i++) {
    hash[array[i] - 1]++;
  }
  return hash;
}
console.log(frequencyCount([2, 3, 2, 3, 5])); // Output: [0, 2, 2, 0, 1]
console.log(frequencyCount([3, 3, 3, 3])); // Output: [0, 0, 4, 0]
console.log(frequencyCount([1])); // Output: [1]
